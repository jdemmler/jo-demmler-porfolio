<!doctype html>
<html>
<head>
	<meta charset="utf-8">
	<title>Jo Demmler</title>
	<meta name="viewport" content="width=device-width">
	<link rel="stylesheet" type="text/css" href="styles/styles.css">
<!--[if lt IE 9]>
<script src="http://html5shiv.googlecode.com/svn/trunk/html5.js"></script>
<![endif]-->
<link href='http://fonts.googleapis.com/css?family=Lato:400,700,900,700italic' rel='stylesheet' type='text/css'>
<script src="//use.typekit.net/iir4kot.js"></script>
<script>try{Typekit.load();}catch(e){}</script>
</head>

<body id="back_top_anchor">

	<header>
		<div class="inner-header">

			<div class="logo">

				<a href="#"><img src="images/jd-logo-v3.png" alt="Jo Demmler Web Design Logo"></a>

			</div> --><!-- end logo -->

			<div class="tagline">
				<h1>Interactive Web Design & Development</h1>
				<span class="divide-line-header"></span>
				<h2>specialising in travel, tourism, adventure & active pursuits</h2>
			</div><!-- end tagline -->

			<nav>
				<ul>
					<div class="about">
						<li class="accordion-title">About<span class="plus">  +</span></li>
						<div class="show-hide">
							<p>A passionate and dynamic web designer specializing in travel, tourism, adventure & active pursuits with 10 years international experience within the travel & tourism industry. Hands on project experience designing and proven skills in online marketing strategies, project management, user experience design and usability testing.</p> 

							<p>Known for undertaking unique adventures including cycling from Istanbul to Beijing as well as from Singapore to Beijing.</p> 

							<p>Builds and develops custom websites which are responsive, beautiful in design and reaches the needs of the client. Constant learner with Masters of Business Administration.</p>
						</div><!-- end show-hide -->	
					</div><!-- end about -->
					<li><a href="#featured">Featured Projects</a></li>
					<li><a href="#lets-talk">Get In Touch</a></li>
				</ul>
			</nav>


		</div><!-- end inner-header -->
		<p class="image-credit"><a class="white" href="http://www.flickr.com/photos/9135956@N04/3650237678" target="blank">Image Credit: Shelby H. The World</a></p>
	</header>


	<div class="wrapper">

		<main>

			<section class="featured">
				<h1 id="featured">Featured Projects</h1>

				<article class="featured-container ptm">


					<div class="featured-description">
						<h2>Personal Travel Management Ltd</h2>
						<p>currently under construction</p> 
						<p>Personal Travel Management is an independent travel agency located in Vancouver, proudly hosting twenty six travel professionals throughout British Columbia and Alberta. They required a website that showcased their travel experts, their travel specialities and their group departures, while at the same time inspiring customers with beautiful and engaging imagery.</p>
						<span class="divide-line-content"></span>
						<ul class="skills">
							<li>Wordpress</li>
							<li>Information Architecture</li>
							<li>User Expereince Design</li>
							<li>HMTL5, CSS3, PHP, jQuery</li>
							<li>Responsive Design</li>
						</ul>
						<p><a class="view-project" href="http://ptm.htpwebdesign.ca/" target="blank">View project</a></p>

					</div><!-- end featured description -->

					<div class="featured-image-container">
						<a href="http://ptm.htpwebdesign.ca" target="blank">
							<img src="images/desk-concept-ptm-sm.jpg" alt="Image of Personal Travel Managements Home page">
						</a>
						</div><!-- end featured-image-contaner -->

					</article><!-- featured-container end ptm -->


					<article class="featured-container bcit">

						<div class="featured-description">
							<h2>BCIT</h2>
							<p>BCIT School of Computing and Academic Studies: High-Tech Professional Program required a website to showcase the possibilities for companies and individuals to work with BCIT students. The site needed to prioritize the content, have a simple navigation and make it easy for employers to review options and to contact BCIT with any inquiries.</p>
							<span class="divide-line-content"></span>
							<ul class="skills">
								<li>Information Architecture</li>
								<li>User Expereince Design</li>
								<li>HMTL5, CSS3, PHP, PHP Templates, jQuery</li>
								<li>Responsive Design</li>
							</ul>
							<p><a class="view-project" href="http://localhost/twd07-3c/htp-employer-site/index.php" target="blank">View project</a></p>

						</div><!-- end featured description -->

						<a href="http://localhost/twd07-3c/htp-employer-site/index.php" target="blank">
							<div class="featured-image-container">
								<img src="images/bcit-macbook.jpg" alt="Image of BCIT High Tech Proffesional Home page for work terms and projects"></a>
							</div><!-- end featured-image-contaner -->


						</article><!-- featured-container end bcit -->


						<article class="featured-container thor">

							<div class="featured-description">
								<h2>Thor Travel</h2>
								<p>Thor Travel is boutique adventure travel agency located in Adelaide, Australia. This was a school project highlighting the possibilities available to enhance their current website.</p>
								<span class="divide-line-content"></span>
								<ul class="skills">
									<li>User Expereince Design</li>
									<li>HTML5, CSS3</li>
								</ul>
								<p><a class="view-project" href="http://jdemmler.htpwebdesign.ca/thor_travel/index.html" target="blank">View project</a></p>
							</div><!-- end featured description -->

							<a href="http://jdemmler.htpwebdesign.ca/thor_travel/index.html" target="blank">
								<div class="featured-image-container">
									<img src="images/thor-macbook.jpg" alt="Image of Thor Travels Home page">
								</a>
							</div><!-- end featured-image-contaner -->


						</article><!-- featured-container end thor -->


					</section><!-- end featured-->

					<section class="get-in-touch">
						<h1 id="lets-talk">Get in Touch</h1>
						<span class="divide-line-content"></span>
						<p>Always happy to chat!</p>
						<i class="fa fa-envelope"></i>
						<p><a href="mailto:joannedemmler@gmail.com">joannedemmler@gmail.com</a></p>
						<a href="https://www.linkedin.com/profile/view?id=92115337&trk=nav_responsive_tab_profile" target="blank">
							<i class="fa fa-linkedin-square"></i>
						</a>
					</section>	

				</main>
			</div><!-- end wrapper -->

			<footer>
				<p><i class="fa fa-copyright"></i> 2014 Design & Development Jo Demmler</p>
				<div class="clear">
				</div>

				<div class="back">
					<a href="#back_top_anchor" class="scrollup">
						<i class="fa fa-chevron-circle-up"></i>
					</a>
				</div>
			</footer>



			<script src="//ajax.googleapis.com/ajax/libs/jquery/2.1.1/jquery.min.js"></script>
			<script src="scripts/scripts.js"></script>

		</body>
		</html>